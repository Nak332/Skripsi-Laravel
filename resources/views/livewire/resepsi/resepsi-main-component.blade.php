    ...
    <div id="header-page" x-data="{ expanded: false }" @click="expanded = !expanded"  @click.outside="expanded = false" class="flex justify-center">
      ...
      <div class="md:flex " >
            <div x-show="expanded"  x-collapse class="flex {{$current_patient ? 'hidden' : ''}}">
                @if ($appointment)
                <button  x-on:click="$wire.undo(id='{{$appointment->id}}')" wire:click="$refresh()"  title="Kembalikan pasien sebelumnya" class="w-20 h-20  bg-yellow-300 rounded-lg text-xl font-bold items-center p-4 ml-4 mt-6 drop-shadow-md  outline-offset-1 ">
